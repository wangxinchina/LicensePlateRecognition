...
# def binaryPic(upper,lower,dila_times,ero_times):
#     # 把范围内颜色转为白色，其他颜色转为黑色
...
def roughCutPic(imgnum,upper,lower,dila_times,ero_times):
    ...
    # else:
    #     # 读取处理一次的图片
    #     imgb = cv2.imread(os.path.dirname(__file__) + '/test_images/cut.png')
    #
    # 把范围内颜色转为白色，其他颜色转为黑色
...
if __name__=="__main__":
    with open('pathpic.txt','r') as f:
        path=f.read()
    print(path)
    roughCutPic(path, upper_blue, lower_blue, 8, 2)
    print('ok')
